...
desiredCaps.name= 'UC000059 - SUPERVISOR - Create Schedule';
...
.then(async function() {
  ...
  if (desiredCaps.platformName == 'iOS' || desiredCaps.platformName == 'ios' || desiredCaps.platformName == 'IOS')
	...
	.then(async function(){
		console.log(viewClick+'View Click');
		if(viewClick)
		{
			driver
			.sessionCapabilities()
	...
	.then(async function() {
		...
		{ 
			isExists = await driver.elementByXPathIfExists('//android.widget.TextView[@text="Sunday"]', 4000);
			console.log('isExists'+isExists);
			return await driver.waitForElementByXPath('//android.widget.TextView[@text="Sunday"]', 4000);
	...
	.then(async function() {
		...
		console.log('Day Name : Sunday');
		if (desiredCaps.platformName == 'iOS' || desiredCaps.platformName == 'ios' || desiredCaps.platformName == 'IOS')
			return await driver.elementByAccessibilityIdIfExists(
				'Sunday ',
				asserters.isDisplayed && asserters.isEnabled,
				4000
			);
		else return await driver.elementByXPathIfExists('//android.widget.TextView[@text="Sunday"]', 4000);
	} catch (err) {}
	})
	.then(async function(searchElement) {
		try{
	...
	.fin(function() {
		try {
			console.log('Usecase Executed : UC000059');
			return driver.quit();
		} catch (e) {
			console.log('Usecase failed : UC000059');
			console.log(e);
		}
	})
	.done();
