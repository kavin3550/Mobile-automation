...
desiredCaps.name = 'UC000036 - Leadership - Flexi Checkin and Checkout resides in dashboard';
...
driver
	...
	.then(function() {
		...
		driver
			...
			else return driver.waitForElementByXPath('//android.widget.Button[@text="Allow" or @text="ALLOW"]', asserters.isEnabled, 4000);
			})
			.then(function(searchElement) {
				return searchElement.click();
			});
	...
	.then(async function() {
		...
		if (desiredCaps.platformName == 'iOS' || desiredCaps.platformName == 'ios' || desiredCaps.platformName == 'IOS')
			return await driver.waitForElementByXPath(
				'//XCUIElementTypeOther[@name="checkout"]',
				asserters.isDisplayed && asserters.isEnabled,
				4000
			);
		else
			return await driver.waitForElementByXPath(
				'//android.view.ViewGroup[@content-desc="checkout"]',
				asserters.isEnabled,
				4000
			);
		} catch (err) {}
	...
	.fin(function() {
		...
		try {
			console.log('Usecase Executed : UC000036');
			return driver.quit();
		} catch (e) {
			console.log('Usecase failed : UC000036');
			console.log(e);
		}
	})
	.done();
