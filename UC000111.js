...
desiredCaps.name= 'UC000111 - SUPERVISOR - Add Leave and Manager Chat';
...
driver
	...
	.then(async function() {
		...
		console.log('Leave type - Selection');
			if (desiredCaps.platformName == 'iOS' || desiredCaps.platformName == 'ios' || desiredCaps.platformName == 'IOS')
			return await driver.waitForElementByXPath(
				'(//XCUIElementTypeOther[@name="Pilgrim holiday"])[1]',
				asserters.isDisplayed && asserters.isEnabled,
				25000
			);
			else 
			return await driver.waitForElementByXPath(
				...
				'//android.widget.TextView[@text="Pilgrim holidays" or @text="Pilgrim holiday"]',
				asserters.isEnabled,
				25000
			);
		} catch (err) {}
	...
	.then(async function() {
		...
		else return await driver.waitForElementByAccessibilityId('Kumaravel Thangaraj'+day_string+' '+month+' '+year, asserters.isEnabled, 4000);
	} catch (err) {}
	})
	.then(async function(searchElement) {
		return await searchElement.click();
	...
	.fin(function() {
	  ...
	  try {
		console.log('Usecase Executed : UC000111');
		return driver.quit();
	} catch (e) {
		console.log('Usecase failed : UC000111');
		console.log(e);
	}
	})
	.done();
