...
desiredCaps.name = 'UC000115 - Employee - Add Claim and Leadership Chat'
...
.then(async function() {
  ...
  if (desiredCaps.platformName == 'iOS' || desiredCaps.platformName == 'ios' || desiredCaps.platformName == 'IOS')
	...
	.then(async function() {
		...
		else return await driver.waitForElementByXPath('//ancestor::*[*[@text="My Claims" or @text="My claims"]]', asserters.isEnabled, 10000);
	})
	.then(async function(searchElement) {
		return await searchElement.click();
	})
	...
	.then(async function() {
		...
		else return await driver.elementByXPathOrNull('//android.view.ViewGroup[@content-desc=\"Yogaraj M'+day_string+' '+month+' '+year+'\"]', asserters.isEnabled, 25000);
	})
	.then(async function(searchElement) {
		if(searchElement)
		{
	...
	.then(async function() {
		...
		//else return await driver.waitForElementByAccessibilityId('Yogaraj M-Food-175.00-30 March 2022', asserters.isEnabled, 25000);
		else return await driver.elementByXPathOrNull('(//android.view.ViewGroup[@content-desc=\"Yogaraj M'+day_string+' '+month+' '+year+'\"])[1]', asserters.isEnabled, 25000);
	})
	.then(async function(searchElement) {
		if(searchElement)
		{
	...
	.then(async function () {
	  ...
	  .then(async function(searchInput) {
		if(searchInput)
		  return await searchInput.sendKeys('Chat by - Leadership');
	  })
	...
	.fin(function() {
	  ...
	  try {
		console.log('Usecase Executed : UC000115');
		return driver.quit();
	} catch (e) {
		console.log('Usecase failed : UC000115');
		console.log(e);
	}
	})
	.done();
